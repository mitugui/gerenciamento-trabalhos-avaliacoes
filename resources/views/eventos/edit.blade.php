<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
            <h3 class="text-lg font-bold mb-4">Editar Evento</h3>
            ...
            <form method="POST" action="{{ route('eventos.update', $evento->id) }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="datetime-local" name="data_inscricao" id="data_inscricao" required
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                            value="{{ old('data_inscricao', \Carbon\Carbon::parse($evento->data_inscricao)->format('Y-m-d\TH:i')) }}">
                    </div>
                    <!-- Data de Início -->
                    <div class="mb-4">
                        ...
                        <input type="datetime-local" name="data_inicio" id="data_inicio" required
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                            value="{{ old('data_inicio', \Carbon\Carbon::parse($evento->data_inicio)->format('Y-m-d\TH:i')) }}">
                    </div>
                    <!-- Data de Fim -->
                    <div class="mb-4">
                        ...
                        <input type="datetime-local" name="data_fim" id="data_fim" required
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                            value="{{ old('data_fim', \Carbon\Carbon::parse($evento->data_fim)->format('Y-m-d\TH:i')) }}">
                    </div>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="descricao" id="descricao" rows="3"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('descricao', $evento->descricao) }}</textarea>
                </div>
